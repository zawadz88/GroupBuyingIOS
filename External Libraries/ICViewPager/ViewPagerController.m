...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    for (id<ViewPagerIndicatorDelegate> indicator in self.indicators) {
        if ([indicator respondsToSelector:@selector(viewPager:contentChangedToIndex:)]) {
            [indicator viewPager:self contentChangedToIndex:self.activeTabIndex];
        }
    }
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed {
    ...
    NSLog(@"New tab: %d", self.activeTabIndex);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (![self isAnimatingToTab]) {
        ...
        NSLog(@"Moved ratio: %f", movedRatio);
        
        for (id<ViewPagerIndicatorDelegate> indicator in self.indicators) {
            if ([indicator respondsToSelector:@selector(viewPager:contentMovedByRatio:)]) {
                [indicator viewPager:self contentMovedByRatio:movedRatio];
            }
        }
    }        
